...
import { DanhMucKhoaHoc, KhoaHoc } from '../../types/course.type'
import { InfoUser } from '../../types/user.type'
const Header = () => {
  ...
  const [listKhoaHoc, setListKhoaHoc] = useState<[] | undefined>()
  ...
  const [dataUser, setDataUser] = useState<InfoUser | null>()

  const renderCourse = (arr: DanhMucKhoaHoc[]) => {
    return arr.map((item: DanhMucKhoaHoc) => {
      return (
              ...
              >
                ...
                {key && (
                  <div className={classes.list_khoahoc}>
                    {listKhoaHoc && listKhoaHoc?.length > 0 ? (
                      listKhoaHoc?.map((item: KhoaHoc, index) => {
                        return (
                          <div
                            key={index}
      ...
      >
        <Box sx={style}>
          ...
          <Grid container>
            ...
            <Grid item xs={6}>
              {dataUser && dataUser?.chiTietKhoaHocGhiDanh?.length > 0 ? (
                dataUser?.chiTietKhoaHocGhiDanh?.map((item: KhoaHoc, index) => {
                  return (
